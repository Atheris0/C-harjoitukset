...
namespace VendingMachine
{
    ...
    public class VendingMachine
    {
        ...
        {
            if (insertedCash >= bottlePrice && !IsSoldOut())
            {
                bottleCount--;
                double change = insertedCash - bottlePrice;
                earnedCash += insertedCash - change;
                insertedCash = 0;
                return change;
            }
            else { return -1; }
        ...
        {
            return $"Bottles left: {bottleCount}\nBottle price: {BottlePrice} euroa\nMoney earned: {EmptyCashbox()} euroa";
